/* 
  The structure of the data received from the database is a superset of the Result interface, 
  which means that data received from the database can be used without needing further processing.
*/
...
export class SqlRepository implements Repository {
  ...
  executeQuery(sql: string, params: any): Promise<Result[]> {
    return new Promise<Result[]>((resolve, reject) => {
      // The all method executes a SQL statement with an optional set 
      // of parameters (also satinize them) and passes all result 
      // rows to the callback function as an array of type Result.
